...
class Register extends Component {
  ...
  getUserValidation() {
    if (validator.isAlpha(this.state.userName)) return 'success';
    else if (!(validator.isAlpha(this.state.userName))&& this.state.userName.length > 0) return 'error';
    return null;
  }
    ...
    if(localStorage.getItem("user_id")!==null) {
      if (!(validator.isEmail(localStorage.getItem("email_id")) && ((this.state.userName.length > 3) && (this.state.userName.length < 15)) && (/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,15}/.test(this.state.password)))) {
        this.setState({
          ...
          axios.put(`${config.url.user}/${localStorage.getItem("user_id")}`, {
      ...
      if (!(validator.isEmail(this.state.email) && (validator.isAlpha(this.state.userName)) && (/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,15}/.test(this.state.password)))) {
        this.setState({
          submit: false
        });
      } else {
