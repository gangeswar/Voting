...
class QuestionManage extends Component {
  ...
  componentWillMount() {
    axios.get(`${config.url.question}`).then(res => {
      this.setState({
  ...
  onClickDeleteTotalQuestion(cell, row, rowIndex, totalQuestion) {
    axios.get(`${config.url.question}/${totalQuestion._id}/option`).then(res => {
      for (let index of res.data) {
        axios.delete(`${config.url.option}/${index._id}`);
      }
    }).then(res => {
      axios.delete(`${config.url.question}/${totalQuestion._id}`).then(res=>{
        axios.get(`${config.url.question}`).then(res => {
          this.setState({
            totalQuestion: res.data,
            renderedQuestions: res.data.slice((this.state.page - 1) * 5, (this.state.page - 1) * 5 + 5),
  ...
  onClickEditQuestion(cell, row, index) {
    ...
    axios.get(`${config.url.question}/${this.state.renderedQuestions[index]._id}/option`).then(res => {
      this.setState({
        editOption: res.data
      });
